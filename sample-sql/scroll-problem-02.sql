...
BEGIN
    ...
    FOR week_num IN 0..26 LOOP
        RETURN QUERY
        SELECT
            week_num,
            CASE
                WHEN cohort_size = 0 THEN 0.00
                ELSE ROUND(
                    100.0 * COUNT(DISTINCT s.user_id) / cohort_size, 2
                )
...
BEGIN
    ...
    WHERE NOT EXISTS (
        ...
        WHERE s.user_id = prev.user_id
          AND s.started_at::date BETWEEN target_month
    ...
    SELECT
        ...
        CASE
            WHEN active_prev = 0 THEN 0.00
            ELSE ROUND(100.0 * churned_count::NUMERIC / active_prev, 2)
        END AS churn_rate;
END;
$$ LANGUAGE plpgsql;
